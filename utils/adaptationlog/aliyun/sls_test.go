...
func TestWithProject(t *testing.T) {
	...
	funcProject(opts)
	if opts.projectName != project {
		t.Errorf("WithProject() = %s, want %s", opts.projectName, project)
	}
}
...
func TestWithLogstore(t *testing.T) {
	...
	funcLogstore(opts)
	if opts.logStorage != logstore {
		t.Errorf("WithLogatore() = %s, want %s", opts.logStorage, logstore)
	}
}
