...
func TestFilterAll(t *testing.T) {
	logger := WithLog(DefaultLogger, "ts", DefaultTimestamp, "caller", DefaultCaller)
...
func TestFilterFuncWitchLoggerPrefix(t *testing.T) {
	...
	tests := []struct {
		...
		{
			logger: NewFilter(WithLog(NewStdLogger(buf), "caller", "caller", "prefix", "whaterver"), FilterFunc(testFilterFuncWithLoggerPrefix)),
			want:   "",
		},
		{
			// Filtered value
			logger: NewFilter(WithLog(NewStdLogger(buf), "caller", "caller"), FilterFunc(testFilterFuncWithLoggerPrefix)),
			want:   "INFO caller=caller msg=msg filtered=***\n",
		},
		{
			// NO prefix
			logger: NewFilter(WithLog(NewStdLogger(buf)), FilterFunc(testFilterFuncWithLoggerPrefix)),
...
func testFilterFuncWithLoggerPrefix(level Level, kvs ...interface{}) bool {
	if level == LevelWarn {
		return true
	}
